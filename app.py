...
ORANGE = "#FF441F"
ORANGE_MID = "#FF8C00"
ORANGE_PALE = "rgba(255,68,31,0.15)"
DARK = "#1A1A2E"
...
# Resampled series for the main time-series chart
df_res = (
...
with col_title:
    st.markdown("# Rappi Store Monitor")
    st.caption(
        "Dashboard de Disponibilidad de Tiendas · `synthetic_monitoring_visible_stores`"
    )
st.divider()
...
with tab_dash:
    ...
    with col_a:
        ...
        fig_hm.update_layout(
            ...
            yaxis=dict(title="Hora del dia", tickmode="linear", dtick=2, autorange="reversed"),
    ...
    with col_c:
        ...
        fig_hp = go.Figure()
        fig_hp.add_trace(
            go.Scatter(x=hrs, y=mu + sigma, line=dict(width=0), showlegend=False, mode="lines")
        )
        fig_hp.add_trace(
            go.Scatter(
                x=hrs,
        ...
        if not anomalies.empty:
            fig_ano.add_trace(
                go.Scatter(
                    ...
                    y=anomalies["stores"],
                    mode="markers",
                    name=f"Anomalia (|z|>{anomaly_sigma})",
    ...
    with col_f:
        ...
        dow_avg = (
            df.groupby("day_name")["stores"]
            .mean()
            .reindex(dow_order)
            .dropna()
        )
        bar_colors = [
            ORANGE if d not in ("Saturday", "Sunday") else DARK for d in dow_avg.index
        ]
...
with tab_chat:
    if not api_key:
        st.warning(
            "Ingresa tu Gemini API Key en la barra lateral para activar el asistente."
        )
        st.stop()
    ...
    col_chat_title, col_chat_logo = st.columns([11, 1])
    with col_chat_title:
        st.markdown("### Asistente de Datos Rappi")
        st.caption("Pregunta cualquier cosa sobre la disponibilidad de tiendas.")
    with col_chat_logo:
